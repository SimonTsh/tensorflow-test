...
function convertToTensor(data) {
	...
	return tf.tidy(() => {
		...
		return {
			...
			// Return the min/max bounds so that we can use them later
			inputMin,
			inputMax,
			labelMin,
			labelMax,
...
async function trainModel(model, inputs, labels) {
	...
	model.compile({
		optimizer: tf.train.adam(), // adam optimizer - quite effective in practice and requires no configuration
		loss: tf.losses.meanSquaredError,
		metrics: ['mse'],
	});

	const batchSize = 32; // common batchSize is between 32 to 512
	const epochs = 50;
...
async function run() {
	...
	// Convert data to a form we can use for training
